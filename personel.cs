...
namespace SQL
{
    public partial class personel : Form
    {
        ...
        {
            baglanti.Open();
            NpgsqlCommand ekle = new NpgsqlCommand("insert into \"personel\" (\"personelNo\",\"personelAdi\",\"personelSoyadi\",\"calistigiSube\") values (@p3,@p1,@p2,@p4)", baglanti);
        ...
        {
            string listele = "select * from \"personelleriListele\"";
        ...
        private void PersonelSil_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Personel numarasıan kabul edilebilir bir değer giriniz.");
            }
            else
            {
                baglanti.Open();
                NpgsqlCommand sil = new NpgsqlCommand("delete from \"personel\" where \"personelNo\"=@p1", baglanti);
        ...
        private void PersonelAra_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Personel ad ve soyad bilgilerine kabul edilebilir değerler giriniz.");
            }
            else
            {
                baglanti.Open();
                DataTable tbl = new DataTable();
                NpgsqlDataAdapter ara = new NpgsqlDataAdapter("select * from \"personelAra\"('%" + textBox1.Text + "%','%" + textBox2.Text + "%')", baglanti);
                ara.Fill(tbl);
                baglanti.Close();
        ...
        {
            baglanti.Open();
            NpgsqlCommand guncelle = new NpgsqlCommand("update \"personel\" set \"personelAdi\"=@p1,\"personelSoyadi\"=@p2,\"calistigiSube\"=@p4 where \"personelNo\"=@p3", baglanti);
